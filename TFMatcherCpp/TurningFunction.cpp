...
namespace TFMatcherCpp {
	...
	double TurningFunction::GetComplexity() {
		return complexity;
	}
	...
	double TurningFunction::ValueAt(double _x) const
	{
		...
		size_t low = 0, high = polygonLength - 1;

		//while ((mid >= 0) || (mid < polygonLength))
		while (low <= high && high < polygonLength && low >= 0)
		{
			int mid = (low + high) / 2;
